...
/**
 * hh l1mintToken721 --exampletoken 0x6342B9D2A6507136e278704aADe193F84D88671e --tokenurl 0002 --to 0xD6f15EAC1Cb3B4131Ab4899a52E711e19DEeA73f --network opl1
 */
 ...
 * hh approveToken721 --exampletoken 0x6342B9D2A6507136e278704aADe193F84D88671e --l1erc721gateway 0xafbEa3773092Eb8Da018693133eF2720CA46D115 --to 0xD6f15EAC1Cb3B4131Ab4899a52E711e19DEeA73f --network opl1
 */
task("approveToken721", "Mint Token")
    .addParam("exampletoken", "ExampleToken address")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        // 授权合约，通过网管合约转帐
        const approve=await ERC721.approve(taskArgs.l1erc721gateway, 2)
        await approve
 ...
 * hh depositToken721 --l1erc721gateway 0xafbEa3773092Eb8Da018693133eF2720CA46D115 --tokenid 2 --network opl1
 */
...
/**
 * 确认l2账户
 * hh l2721balances --token 0x7396ba598A935eBccDC841C8d1814F04D2B47Cee --to 0xD6f15EAC1Cb3B4131Ab4899a52E711e19DEeA73f --network opl2
 */
task("l2721balances", "确认l2账号信息")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        const token = await tokenFactory.attach(taskArgs.token)
            ...
            const ownerOfAddr= await token.ownerOf(2)
            console.log("ownerOfAddr->", ownerOfAddr)
    });
...
task("opTransfer721", "转账")
    ...
    .setAction(async (taskArgs, hre) => {
            ...
            const  balanceOf= await token.balanceOf(taskArgs.to)
            console.log("balanceOf->",await balanceOf)
            const txInfo= await token.transferFrom(taskArgs.from, taskArgs.to, 2)
